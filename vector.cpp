...
{
    Vector to_app_point(app_point.cx, app_point.cy, app_point.cz, sys);
...
void Vector::createArrow()
{
    ...
    auto left_line  = Vector(cos30 * (getX() - capp_point.cx) + sin30 * (getY() - capp_point.cy),
                            -sin30 * (getX() - capp_point.cx) + cos30 * (getY() - capp_point.cy));
    left_line = left_line.normalize() * 10;

    auto right_line = Vector(cos30 * (getX() - capp_point.cx) - sin30 * (getY() - capp_point.cy),
                             sin30 * (getX() - capp_point.cx) + cos30 * (getY() - capp_point.cy));
